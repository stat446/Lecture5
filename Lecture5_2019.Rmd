...
Suppose the population is partioned into disjoint sets of sampling units called __strata__. If a sample is selected within each stratum, then this sampling procedure is known as 
\vfill
...
Point (2) implies that only within-stratum variances contribute ot the variance of an estimator. Thus, the basic motivating principle behind using stratification to produce an estimator with small variance is to partition the population so that units within each stratum are as similar as possible. 
...
Because the variance formulas for $\hat{t}_{str}$ and $\hat{\bar{y}}_{U,str}$ are determined only from within-stratum variances, the precision of the estimators can be improved by forming strata with small $S_h^2$ values (strata with similar $y-values$ within each stratum). 
\vfill
If the variance of the stratified sampling estimator is less than the variance of the SRS estimator,
\vfill
A stratified SRS estimator will be more efficient than the SRS estimator of $\bar{y}_U$ or $t$ 
\vfill
...
\vfill
__Situation 1:__ If all of the strata are the same size and no prior information is available about the population, a reasonable choice would be 
\vfill
...
__Situation 2:__ If the strata are not all the same size and no prior information is available about the population, a reasonable choice would be to assign sample sizes 
\vfill
...
#### Quota Sampling
When the population is stratified based on more than one factor we are using __multifactor stratification.__ 
\vfill

A marketing survey was conducted to get information on computer users' use of the internet for shopping. The surveyor suspected there may be differences based on gender (2 levels), househould income (6 levels), and age (5 levels). 
\vfill

__Quota sampling__ is a form of stratified sampling and typically uses multifactor stratification. Taking a quota sample ensures that data are collected across the population with the belief that doing so will provide a representative sample from the population. 
\vfill

So how does quota sampling differ from stratified simple random sampling? 
\vfill
...
Although taking a quota sample can save a lot of time and money when compared to simple random sampling, the reseacher must realize that if quota sampling is used, 
\vfill
...
\vfill
For example, a fish population may be stratified by age class, body length, body weight, or sex. A random sample of fish is collected. Then each sampled fish is examined and its stratum is recorded along with the response of interest. 
\vfill
In many studies a simple random sample is taken from the population and then is stratified. 
\vfill
In constrast to traditional stratified simple random sampling, 
\vfill